...
// check requirements to run Al-Ghazel
...
/**
 * Checks if the current pnpm version is compatible with Al-Ghazel.
...
function checkPnpmVersion() {
  ...
  if (!currentPnpmVersion) {
    console.error(
      `\x1b[31m%s\x1b[0m`,
      `You are running Al-Ghazel from a directory that does not have pnpm installed. Please install pnpm and run "pnpm install" in your project directory.`,
    );
  ...
  if (major < 9) {
    console.error(
      `\x1b[31m%s\x1b[0m`,
      `You are running pnpm ${currentPnpmVersion}. Al-Ghazel requires pnpm ${requiredPnpmVersion}.`,
    );
  ...
  if (minor < 12) {
    console.warn(
      `\x1b[33m%s\x1b[0m`,
      `You are running pnpm ${currentPnpmVersion}. Al-Ghazel recommends using pnpm 9.12.0 or higher.`,
    );
  } else {
    console.log(
      `\x1b[32m%s\x1b[0m`,
...
/**
 * Checks if the current Node version is compatible with Al-Ghazel.
...
function checkNodeVersion() {
  ...
  if (major < 18 || (major === 18 && minor < 18)) {
    console.error(
      `\x1b[31m%s\x1b[0m`,
      `You are running Node ${currentNodeVersion}. Al-Ghazel requires Node ${requiredNodeVersion}.`,
    );
...
function checkPathNotOneDrive() {
  ...
  if (path.includes('OneDrive')) {
    console.error(
      `\x1b[31m%s\x1b[0m`,
      `You are running Al-Ghazel from OneDrive. Please move your project to a local folder.`,
    );
